...
For performance reasons, the bounding box for an object given by `label_value`
is calculated on the lowest-resolution label image by default. This may lead
to imprecise results, as here where the lower part of object 877 is truncated.
To improve the precision of the bounding box calculation, you can manually
select the pyramid level to be used, using the argument `label_pyramid_level`:
